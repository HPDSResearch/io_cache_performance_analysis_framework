...
while kill -0 $pid1 2> /dev/null; do
        cat /proc/enhanceio/En_cache/stats | tee -a $output-cache-state-warmup.txt         
        sleep 10
done
...
fio --filename=/dev/$1 --write_iops_log=fio_log $5  1>& $4-fio_output.txt &
pid2=$!	
...
#mv $output-cache-state-warmup.txt cache_state_warmup/
...
#./cache_state_reporter.sh $output-cache-state-warmup.txt 
...
cd ..
./fio_output_parser.sh $4-fio_output.txt $4-fio-parsed.csv
